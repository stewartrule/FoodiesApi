...
extension Request {
    var businessReviewRepository: BusinessReviewRepository {
        .init(req: self)
    }
}
...
struct BusinessReviewRepository {
    ...
    func getCountFor(businessID: Business.IDValue)
        async throws -> Int
    {
        return try await query()
            .filter(\.$business.$id == businessID)
    ...
    func getAverageRatingFor(businessID: Business.IDValue)
        async throws
        -> Double
    {
        return try await query()
    ...
    func getCountFor(business: Business) async throws -> Int {
        return try await getCountFor(
            businessID: try business.requireID()
        )
    }

    func getAverageRatingFor(business: Business)
        async throws -> Double
    {

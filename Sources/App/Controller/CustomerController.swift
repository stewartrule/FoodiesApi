...
struct CustomerController: RouteCollection {
    ...
    func orders(req: Request) async throws -> Page<Order> {
        guard
            let customerID = req.parameters.get(
                "customerID",
                as: UUID.self
            )
        else {
    ...
    func chats(req: Request) async throws -> Page<Chat> {
        ...
        guard
            let orderID = req.parameters.get(
                "orderID",
                as: UUID.self
            )
        else {
    ...
    func customer(req: Request) async throws -> Customer {
        ...
        guard
            let customer = try await Customer.query(
                on: req.db
            )
            .filter(\.$id == customerID)
            .with(\.$addresses, { addresses in
                addresses.with(\.$postalArea, { postalArea in
                    postalArea.with(\.$city, { city in
                        city.with(\.$province)
                    })
                })
            })
